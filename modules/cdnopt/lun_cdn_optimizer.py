...
<script>
const $=s=>document.querySelector(s);let cfg=null,measurements=[],ranked=[],cancelled=false;const activeControllers=new Set();
...
function render(){const sorted=measurements.filter(x=>x.speed_mbps>0).sort((a,b)=>score(b)-score(a)||b.speed_mbps-a.speed_mbps||a.latency_ms-b.latency_ms);ranked=sorted.filter(x=>x.latency_ms<=cfg.latency_max&&x.speed_mbps>=cfg.speed_min).slice(0,cfg.top_count);const selected=new Set(ranked.map(x=>x.ip));$('#rows').innerHTML=sorted.length?sorted.map((x,i)=>{const qualified=x.latency_ms<=cfg.latency_max&&x.speed_mbps>=cfg.speed_min;const result=selected.has(x.ip)?'<span class="pill">入选</span>':(qualified?'候选':'未达标');return `<tr><td>${i+1}</td><td><code>${esc(x.ip)}</code></td><td>${esc(x.colo||'-')}</td><td>${x.latency_ms.toFixed(0)} ms</td><td>${x.speed_mbps.toFixed(1)} Mbps</td><td>${result}</td></tr>`}).join(''):'<tr><td colspan="6" class="bad">没有完成的带宽测试</td></tr>';$('#applyCount').textContent=ranked.length;$('#apply').disabled=!ranked.length}
function progress(done,total,text){if(cancelled)return;$('#bar').style.width=(total?done/total*100:0)+'%';$('#status').textContent=text}
async function run(){cancelled=false;measurements=[];ranked=[];$('#start').disabled=true;$('#apply').disabled=true;progress(0,cfg.candidates.length,'正在筛选延迟……');const latency=[];await pool(cfg.candidates,cfg.latency_concurrency,async ip=>{try{latency.push(await latencyTest(ip))}catch(e){}},(d,t)=>progress(d,t,`延迟筛选 ${d}/${t}，可用 ${latency.length}`));if(cancelled)return;const eligible=latency.filter(x=>x.latency_ms<=cfg.latency_max);const finalists=diverseFinalists(eligible,cfg.speed_limit);if(!finalists.length){$('#status').innerHTML='<span class="bad">没有延迟合格的 IP，请检查当前网络是否禁止探测域名。</span>';$('#start').disabled=false;return}progress(0,finalists.length,`正在测试 ${finalists.length} 个候选的带宽……`);await pool(finalists,cfg.speed_concurrency,async item=>{try{measurements.push(await speedTest(item))}catch(e){item.speed_mbps=0;measurements.push(item)}},(d,t)=>progress(d,t,`带宽测试 ${d}/${t}`));if(cancelled)return;render();progress(1,1,ranked.length?`完成：${ranked.length} 个节点达到 ${cfg.latency_max}ms / ${cfg.speed_min}Mbps 门槛。`:'已完成，但没有节点同时达到延迟和带宽门槛。');$('#start').disabled=false}
async function apply(){if(!ranked.length)return;$('#apply').disabled=true;const r=await fetch(tokenPath('apply'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({measurements})});const data=await r.json();if(!r.ok){$('#status').innerHTML=`<span class="bad">${data.error||'应用失败'}</span>`;$('#apply').disabled=false;return}$('#status').innerHTML=`<span class="ok">已回传并应用 ${data.selected.length} 个节点，可返回 SSH 终端。</span>`;$('#start').disabled=true}
...
fetch(tokenPath('config')).then(r=>r.json()).then(data=>{cfg=data;$('#candidateCount').textContent=cfg.candidates.length;$('#latencyMax').textContent=cfg.latency_max+' ms';$('#speedMin').textContent=cfg.speed_min+' Mbps';$('#topCount').textContent=cfg.top_count;$('#status').textContent='候选库已就绪，点击“开始综合优选”。'}).catch(()=>{$('#status').innerHTML='<span class="bad">无法读取测速会话，链接可能已过期。</span>'});
    ...
    def do_POST(self):
        ...
        try:
            ...
            selected, accepted = rank_results(
                request.get("measurements", []) if isinstance(request, dict) else [],
                self.server.candidates,
                self.server.latency_max,
                self.server.speed_min,
            ...
            if not selected:
                self._send_json(
                    422,
                    {
                        "error": f"没有节点同时达到 {self.server.latency_max:g}ms / {self.server.speed_min:g}Mbps",
                        "accepted": 0,
                    },
                )
                return
            result = {
                ...
                "latency_max_ms": self.server.latency_max,
                "speed_min_mbps": self.server.speed_min,
