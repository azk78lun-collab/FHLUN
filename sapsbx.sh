...
# Docker 镜像；默认镜像未发布时请通过环境变量覆盖
DOCKER_IMAGE="${DOCKER_IMAGE:-ghcr.io/azk78lun-collab/lun:latest}"
...
for i in "${!CF_USERNAMES[@]}"; do
  ...
  if [ -n "$ROUTE" ]; then
    ...
    push_out="$(cf push "$APP_NAME" --docker-image "$DOCKER_IMAGE" -m 512M --health-check-type port 2>&1)"
    echo "$push_out"
    pushout
    if [ $? -ne 0 ]; then
      continue
