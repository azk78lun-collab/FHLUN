...
服务器总览使用从 `01` 开始的永久编号，状态与地区均显示中文。编号删除后不重排、不复用；并发编号冲突按签名事件顺序收敛。个人、多用户和聚合订阅共用节点名称：`[地区]协议[-线路信息][-地址变体]-服务器编号`，例如 `[德国-法兰克福]vless-xhttp-tls-tcp-01`。同一地区多台 VPS 自动按编号增加地区序号。
...
联邦服务启动、“立即按需同步”和“状态 / 修复”会交叉检查本机公网 IP。只有至少两个固定 HTTPS 探测源得到同一结果且 IP 确实变化时，才更新本机联邦地址并生成由节点本人签名的 `node.metadata` 事件。地址广播沿用 `1→10→100` 两层有界扩散；IP 未变时不创建事件、不联系成员。该流程不改变节点 ID、证书、服务器编号、联邦端口、NAT 映射、协议或 Cloudflare 配置。
