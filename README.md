...
首次在任意 VPS 选择“一键开启本机联邦身份”，会生成独立身份密钥和本机 mTLS 证书。任意成员均可生成有效期 15 分钟、只可使用一次的 `lunjoin://` 加入地址；在另一台已启用的 VPS 粘贴后，Lun 会自动完成信任重载、成员广播、订阅快照补发和二轮事件收敛，无需再到其它服务器操作。大型历史联邦的首次导入可使用最多 120 秒，不再因旧版 15 秒读取限制停在半配对状态。通信只需 TCP：普通 VPS 自动选择未占用高位端口，NAT VPS 只从已有“公网端口-内网端口”映射中分配；Lun 自动放行本机防火墙，默认不使用 443。
