...
“一键优选 CDN 节点”是独立按需模块：只有用户进入该菜单时才下载轻量 Python 程序，平时没有常驻进程。模块默认实时读取每 12 小时聚合一次的 [DustinWin/BestCF](https://github.com/DustinWin/BestCF) 综合/移动/联通/电信地址池，也可切换 CMLiu 与 Cloudflare ASN 原始库；下载失败时使用最近一次成功缓存并明确标记。VPS 会启动一个带随机 token、默认 15 分钟过期的临时页面，分别生成“客户端线路 → Cloudflare”和“本机 VPS → Cloudflare”两个独立榜单，不进行隐藏加权。页面支持 IPv4/IPv6、CIDR、IP 区间、附带端口备注、实时进度、筛选排序、复制、CSV 及跨榜选择；非中国网络只提示，不阻止使用 VPS 榜。应用前必须查看新增/保留/删除差异，确认后才替换现有 `cfip`/`~/lun/cdnip` 并重建订阅，测试端口不会写入优选 IP 列表。
...
在线更新以当前 `lun` 命令实际指向的文件为准；普通用户调用系统 `/usr/bin/lun` 时，会在免密 `sudo` 可用的情况下原子更新该入口，并同步已有的 `~/bin/lun` 副本，避免“更新了另一份文件、再次运行仍是旧版”。本机更新固定使用 FHLUN 官方 `main`，不会被遗留的 `lunurl` 环境变量带回旧地址；也可运行 `lun self-update` 做非交互更新。
...
主菜单第 6 项是独立的可选模块。只有首次进入并确认安装后才会增加 Python 3、SQLite 用户数据库和 `lun-agent` 服务；不安装或停用模块时，普通、未加入分布式集群的单用户协议、UUID 和 BusyBox 订阅行为保持不变。加入分布式集群的单用户例外：聚合订阅必须由 `lun-agent` 的订阅专用模式承载，但不会启用用户、额度、协议权限或流量管理。模块要求 systemd 或 OpenRC，无 init 环境会拒绝安装。
...
主菜单第 8 项默认关闭。启用后每台 VPS 都是对等成员：任意可信成员可在结构化白名单内管理其它成员，但不开放任意 Shell、不依赖 SSH，也不使用固定心跳。配置、用户、额度、授权与订阅快照通过每节点签名的事件链最终一致；协议端口、NAT、证书、Cloudflare 配置始终只保留在各自 VPS。

首次在任意 VPS 选择“一键开启本机联邦身份”，会生成独立身份密钥和本机 mTLS 证书。任意成员均可生成有效期 15 分钟、只可使用一次的 `lunjoin://` 加入地址；在另一台已启用的 VPS 粘贴后，Lun 会自动完成动态信任加载、成员广播、地区识别和订阅快照补发，无需再到其它服务器操作或逐台重启。收尾采用固定宽度 10 的 `1→10→100` 两层有界扩散：每批有唯一批次号和 180 秒截止时间，事件按签名哈希去重，最后只执行一轮收敛检查，避免重复全网广播与无响应等待。大型历史联邦的首次导入可使用最多 120 秒，不再因旧版 15 秒读取限制停在半配对状态。通信只需 TCP：普通 VPS 自动选择未占用高位端口，NAT VPS 只从已有“公网端口-内网端口”映射中分配；Lun 自动放行本机防火墙，默认不使用 443。

旧式 API v2 集群首次进入会显示安全迁移。迁移只转换本机状态，保留服务器编号、用户、Token 与订阅快照；不会复制旧共享 CA 私钥，也不会自动同步、刷新或更新旧成员。旧 API v2 记录显示为“待重新配对/未信任”，需要显式 `add-peer` 后才会激活。Shell 只接受后端 `nodes` JSON 中明确返回 `trusted=true` 的目标；未信任记录不可远控、不可同步 CDN 优选池，也不会进入一键更新。

服务器总览使用从 `01` 开始的永久编号，状态与地区均显示中文。双栈 VPS 会同时显示已交叉确认的 IPv4 和 IPv6，IPv4 排在前面并作为首选联邦通信地址；只有一种地址时只显示该地址。编号删除后不重排、不复用；并发编号冲突按签名事件顺序收敛。个人、多用户和聚合订阅共用节点名称：`[地区]协议[-线路信息][-地址变体]-服务器编号`，例如 `[德国-法兰克福]vless-xhttp-tls-tcp-01`。同一地区多台 VPS 自动按编号增加地区序号。

协议配置仅在目标 VPS 生效；重建成功后本机异步发布签名订阅快照。每台 VPS 使用相同聚合 Token，但输出自己的可达域名/IP 和公网订阅端口。启用联邦或完成旧版迁移时，若完整多用户模块未启用，Lun 会以现有 UUID、订阅 Token、NAT 内网/公网端口、Host 和可用 HTTPS 证书初始化 `lun-agent` 的订阅专用模式，停止旧 BusyBox 并清理其 cron/OpenRC 自启动。接管过程会保存原配置、服务、端口文件和 BusyBox 状态；任一步失败都会恢复原订阅，联邦不会半启用。以后启用完整多用户时会清除“仅订阅”标记并复用原 UUID/Token；停用联邦则恢复原 BusyBox，已经启用的完整多用户服务不受影响。

客户端 GET 由 `lun-agent` 立即返回最近可信聚合缓存，响应完成后才异步调用后端 `subscription-access` 补齐事件；同一 Token 在进程内 30 秒防抖，因此成员网络变慢或临时分区不会拖住订阅下载。服务启动和手动同步也会补齐，但没有固定心跳。

联邦服务启动、“立即按需同步”和“状态 / 修复”会分别交叉检查本机公网 IPv4 与 IPv6。任一地址族只有至少两个固定 HTTPS 探测源得到相同结果才会公布；IPv4 优先，双栈地址一起写入由节点本人签名的 `node.metadata` 事件。地址广播沿用 `1→10→100` 两层有界扩散；地址未变时不创建事件、不联系成员。该流程不改变节点 ID、证书、服务器编号、联邦端口、NAT 映射、协议或 Cloudflare 配置。联邦监听使用 IPv4/IPv6 双栈，TLS 握手在独立工作线程中限时完成，外部慢连接不会再堵塞其它成员更新。

远程动作仅限协议重建、服务管理、内核更新、防火墙、快照/恢复和脚本下发等固定白名单。批量任务先执行一台金丝雀，成功后最多 3 台并行；失败会回滚本轮已成功成员。移除成员会签发永久撤销墓碑，阻止旧证书和旧事件重放；恢复的已撤销成员必须生成新身份和新加入地址后重新加入。

CDN 优选池同步仅处理本机 `cfip/cdnip` 地址。合并模式为来源优先、追加目标独有地址并去重；覆盖模式完全替换目标列表。它绝不传递 CDN Host、证书、Cloudflare Token、Origin Rules、Tunnel、协议模式或端口。菜单只允许选择 `trusted=true` 的成员，并对每个目标分别调用 `cdn-pool-preview` 显示新增/保留/删除差异；用户逐台确认后才调用 `cdn-pool-sync`。每台目标独立备份、应用、重建和失败回滚，一台失败不会造成其它目标的全局写入。

备份分两类：`federation-data` 联邦数据备份不含私钥，可载入其它成员并合并，绝不替换接收方身份；本机完整 `identity` 身份备份包含密码加密私钥，仅用于本机灾难恢复，`identity-restore` 会拒绝恢复仍在线的重复身份。
...
是否需要 Origin Rules 的判断依据是“Cloudflare 边缘端口是否等于源站公网端口”，不是简单按普通/NAT VPS 区分。普通 VPS 的 `2083 → 2083`、以及服务商已做 `2083 → 内网端口` 的 NAT VPS，都属于同端口 CDN，不需要 Cloudflare 端口改写；但 Host DNS 记录仍必须开启橙云。只有例如边缘 `443 → 源站公网 56567` 这种端口不同时才需要 Origin Rules。
