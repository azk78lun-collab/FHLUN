...
sapsbxinstall(){
URL="https://raw.githubusercontent.com/azk78lun-collab/FHLUN/main/sapsbx.sh"
DEST="$HOME/sapsbx.sh"
...
command -v curl > /dev/null 2>&1 && bash <(curl -Ls https://raw.githubusercontent.com/azk78lun-collab/FHLUN/main/sap.sh) || bash <(wget -qO- https://raw.githubusercontent.com/azk78lun-collab/FHLUN/main/sap.sh)
else
...
quoted=$(printf '%s ' $input)
sed -i "82s/^.*$/DELAPP=\"${quoted% }\"/" $HOME/sapsbx.sh
bash $HOME/sapsbx.sh
sed -i "82s/^.*$/DELAPP=\"\"/" $HOME/sapsbx.sh
else
echo "未安装脚本，请卸载重装"
fi
}
...
echo ""
echo ""
echo "Lun小钢炮脚本-SAP多账户自动部署并保活脚本【VPS】"
echo "版本：V25.10.5"
echo "*****************************************************"
echo "*****************************************************"
